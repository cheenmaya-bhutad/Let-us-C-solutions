...
void printenginepartinfo(struct engineparts list[])
{
    ...
    for(int j=0;j<54;j++)
    {
        if((list[j].serialnum[0]=='B') && (list[j].serialnum[1]=='B') && (list[j].serialnum[1]=='1'))
        {
            i=j;
            do
            {
                ...
                i++;
            }while((list[i].serialnum[0]!='C') && (list[i].serialnum[1]!='C') && (list[i].serialnum[1]!='6'));
        }
        else
        {
            continue;
